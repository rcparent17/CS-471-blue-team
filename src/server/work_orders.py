...
def start_work_order(id):
    ...
    databaseHelper.updateWorkOrder(
        order[0], order[1], order[2], order[3], timeStarted, order[5]
    )
...
def finish_work_order(id):
    ...
    databaseHelper.updateWorkOrder(
        order[0], order[1], order[2], order[3], order[4], timeFinished
    )


def assign_mechanic_work_order(id, worker_name):
    order = databaseHelper.getWorkOrderByID(id)
    databaseHelper.updateWorkOrder(
        order[0], worker_name, order[2], order[3], order[4], timeFinished
    )
